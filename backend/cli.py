...
# Third-party artifacts that ride along inside a dump and must NOT be packed into
# the game image: a group folder like _bundle_ / _bundle_ / _RUNE_ (underscore-wrapped
# group name, holding the .nfo plus the group's tools/unlockers/sample) and loose
# side-car metadata files. None are game data — the PS5 ignores them, but they bloat the
...
def _is_scene_group_dir(dirpath: str, name: str) -> bool:
    """True for an underscore-wrapped scene-group folder (_bundle_, _bundle_, …).
...
def _strip_junk_files(root: Path) -> int:
    ...
    (2) third-party artifacts — a ``_GROUP_`` folder (_bundle_, _bundle_, …) and
    ...
    for d in scene_dirs:
        print(f"[INFO] Removed third-party folder '{d}/' before packing "
