...
def _compute_file_storage_worker(
    ...
    if progress_queue is None:
        progress_queue = _PFSC_WORKER_PROGRESS_QUEUE
...
def _init_file_storage_worker(progress_queue: SupportsIntQueue | None) -> None:
    """Share the inherited progress queue with forked file-compression workers."""
    global _PFSC_WORKER_PROGRESS_QUEUE
    _PFSC_WORKER_PROGRESS_QUEUE = progress_queue
...
def build_pfs(
    ...
    if compress and len(compression_file_nodes) > 0:
        ...
        if worker_count == 1 or len(compression_file_nodes) == 1:
            ...
            displayed_progress_units: int = 0
            manager = None
            progress_queue: SupportsIntQueue
            worker_progress_queue: SupportsIntQueue | None
            pool_factory = mp.Pool
            pool_kwargs: dict[str, object] = {}
            if "fork" in mp.get_all_start_methods():
                fork_context = mp.get_context("fork")
                progress_queue = fork_context.Queue()
                worker_progress_queue = None
                pool_factory = fork_context.Pool
                pool_kwargs = {
                    "initializer": _init_file_storage_worker,
            ...
            try:
                ...
                with pool_factory(processes=worker_count, **pool_kwargs) as pool:
                    ...
                    while remaining_results > 0:
                        ...
                        try:
                            ...
                            # ANY worker failure (MemoryError, zlib/struct errors, …),
                            # not just OSError, must clean up the spool files already
                            # written by finished workers — otherwise GB-sized
                            # mkpfs-*.pfsc temp files leak (e.g. on the OOM the app warns
                            # about, since these paths aren't registered for cleanup yet).
                            cleanup_temporary_file_node_payloads(file_nodes=compression_file_nodes)
                            raise
                ...
                if manager is not None:
                    ...
                    with suppress(Exception):
                        progress_queue.close()  # type: ignore[attr-defined]
                    with suppress(Exception):
                        progress_queue.join_thread()  # type: ignore[attr-defined]
            if displayed_progress_units < progress_total_units:
                progress.step(
                    "compress",
                    progress_total_units,
